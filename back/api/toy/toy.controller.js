...
async function getToysStock(req, res) {
  try {
    const stocks = await toyService.getStockByLabels()
    res.send(stocks)
  } catch (err) {
    logger.error('Failed to fetch stock info', err)
    res.status(500).send('Cannot get stock info')
  }
}
